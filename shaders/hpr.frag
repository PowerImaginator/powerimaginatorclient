...
void main(void) {
        ...
        vec3 cam_coarse = texture(u_tex_cam_pos_levels[u_coarse_level], v_tex_coord).xyz; // texture() instead of texelFetch() for linear filtering
	...
	const ivec2 dirs[8] = ivec2[8](
        ...
        for (int s = 0; s < 8; ++s) {
                ...
                for (int dl = sweep_from_level; dl <= sweep_to_level; ++dl) {
                        int L = clamp(/* l + */ dl, 0, u_num_levels - 1);
